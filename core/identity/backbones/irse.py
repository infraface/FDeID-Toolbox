"""
IR / IR-SE backbone (Improved-ResNet, optionally with Squeeze-and-Excitation).

This is the backbone used by the AdaFace model in this package. As with
``iresnet.py`` it is a *backbone* only and is independent of the training loss:
AdaFace pairs this IR-50 backbone with its adaptive-margin loss, but the same
architecture could be trained with ArcFace, CosFace, etc. Architecture-named
factories are provided via ``build_model`` (e.g. ``ir_50``, ``ir_101``,
``ir_se_50``); depth is selected with ``num_layers``.
"""
...
from torch.nn import (Dropout, MaxPool2d, Sequential, Conv2d, Linear,
                      BatchNorm1d, BatchNorm2d, ReLU, Sigmoid, Module, PReLU)

__all__ = [
    'initialize_weights', 'Flatten', 'LinearBlock', 'GNAP', 'GDC', 'SEModule',
    'BasicBlockIR', 'BottleneckIR', 'BasicBlockIRSE', 'BottleneckIRSE',
    'Bottleneck', 'get_block', 'get_blocks', 'Backbone', 'build_model',
]
