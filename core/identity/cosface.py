"""
CosFace Identity Recognition Module

This module provides the CosFace class for face recognition and verification.
It includes the full CosFace implementation and alignment logic compatible with AdaFace and ArcFace.
CosFace uses large-margin cosine loss for training, but shares the same architecture as ArcFace for inference.
"""
...
# Import alignment utilities from arcface (but define our own backbone)
try:
    from .arcface import (
    ...
    # Fallback: define alignment utilities
    REFERENCE_FACIAL_POINTS = [
...
# Backbone
#
# CosFace = IResNet backbone + additive (large-margin) cosine loss. The backbone
# is architecture-only and identical to the one ArcFace uses; it is defined once
# in core.identity.backbones.iresnet so the network is cleanly separated from the
# loss it was trained with. Re-exported here for backward compatibility.
# =========================================================================
...
# =========================================================================
# CosFace Recognition Result
# =========================================================================

@dataclass
class CosFaceRecognitionResult:
...
class CosFace:
    """
    Face recognizer using CosFace (Large Margin Cosine Loss).

    Extracts face embeddings for recognition and verification tasks.
    Compatible with AdaFace and ArcFace interface for easy switching.

    CosFace and ArcFace share the same architecture; the difference is in the
    training loss function (additive cosine margin vs additive angular margin).
    The underlying network is an IResNet (default IResNet-50, set by
    ``num_layers``) defined in ``core.identity.backbones.iresnet``.
    """

    # The loss this wrapper pairs with the IResNet backbone.
    LOSS = 'cosface'
    ...
    def __init__(
        ...
        """
        Initialize CosFace recognizer.

        Args:
            model_path: Path to pretrained CosFace backbone weights (e.g., backbone.pth)
        ...
        # Build the (loss-agnostic) IResNet backbone -- same network as ArcFace.
        ...
        print(f"[CosFace] Loaded R{num_layers} model on {self.device}")
    ...
    def _load_model(self, model_path: str):
        ...
        print(f"[CosFace] Loading weights from {weight_file}")
        ...
        # Remove common prefixes only (no name mapping needed with official architecture)
        cleaned_state_dict = {}
        ...
        if missing_keys:
            print(f"[CosFace] Warning: Missing keys: {missing_keys[:5]}...")  # Show first 5
        if unexpected_keys:
            print(f"[CosFace] Warning: Unexpected keys: {unexpected_keys[:5]}...")  # Show first 5
    ...
    def align_face(
        ...
        """
        Align face using landmarks (compatible with AdaFace/ArcFace interface).
        ...
        if landmarks is None:
            print("[CosFace] Error: Landmarks are required for alignment.")
            return None
        ...
        if isinstance(image, str):
            ...
            if img_bgr is None:
                print(f"[CosFace] Failed to load image: {image}")
                return None
        ...
        try:
            ...
            print(f"[CosFace] Alignment failed: {e}")
            return None
    ...
    def extract_embedding(
        ...
        Note: CosFace doesn't use adaptive margins, so norm is always 1.0

        Args:
            aligned_face: Aligned face (PIL RGB Image or BGR numpy array)

        Returns:
            Tuple of (embedding, norm) where norm is 1.0 for CosFace
        """
