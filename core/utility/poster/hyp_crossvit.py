"""
Hybrid Vision Transformer for POSTER model.

Implements the pyramid cross-fusion transformer architecture.
"""
...
class Block(nn.Module):
    ...
    def __init__(self, dim, in_chans, q_chanel, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn_img = Attention_img(dim, in_chans=in_chans, q_chanel=q_chanel, num_heads=num_heads,
                                      qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        self.attn_lm = Attention_lm(dim, in_chans=in_chans, q_chanel=q_chanel, num_heads=num_heads,
                                    qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
class PyramidBlock(nn.Module):
    """Pyramid block with multi-scale processing."""
...
class HyVisionTransformer(nn.Module):
    """Hybrid Vision Transformer with pyramid cross-fusion."""
    ...
    def __init__(self, in_chans=49, q_chanel=49, num_classes=1000, embed_dim=512, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            PyramidBlock(
                dim=embed_dim, in_chans=in_chans, q_chanel=q_chanel, num_heads=num_heads,
