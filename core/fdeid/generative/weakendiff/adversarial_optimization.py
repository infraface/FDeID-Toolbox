...
print("DEBUG: Importing utils in adversarial_optimization...", flush=True)
from utils import *
print("DEBUG: Imported utils in adversarial_optimization", flush=True)
...
class Adversarial_Opt:
    ...
    def get_FR_embeddings(self, image):
        ...
        for model_name in self.surrogate_models.keys():
            ...
            # Cast image to match FR model dtype (FR models are in fp32)
            fr_dtype = next(fr_model.parameters()).dtype
            image_for_fr = F.interpolate(image, size=input_size, mode='bilinear').to(dtype=fr_dtype)
            emb_source = fr_model(image_for_fr)
    ...
    def diffusion_step(self, latent, null_context, t,
        ...
        # Get UNet dtype for proper input conversion
        unet_dtype = next(self.diff_model.unet.parameters()).dtype
        original_dtype = latent.dtype

        if not is_null_optimization:
            latent_input = torch.cat([latent] * 2).to(dtype=unet_dtype)
            noise_pred = self.diff_model.unet(
                latent_input, t, encoder_hidden_states=null_context)["sample"]
            noise_pred, _ = noise_pred.chunk(2)
        else:
            noise_pred = self.diff_model.unet(
                latent.to(dtype=unet_dtype), t, encoder_hidden_states=null_context)["sample"]

        # Convert noise_pred back to original dtype for scheduler
        noise_pred = noise_pred.to(dtype=original_dtype)
    ...
    def image2latent(self, image):
        with torch.no_grad():
            ...
            # Ensure image dtype matches VAE dtype
            vae_dtype = next(self.diff_model.vae.parameters()).dtype
    ...
    def ddim_inversion(self, image):
        ...
        for i in tqdm(range(self.diffusion_steps - 1), disable=True):
            t = self.diff_model.scheduler.timesteps[self.diffusion_steps - i - 1]
            #
            # Convert latent to UNet dtype for inference, then convert back
            noise_pred = self.diff_model.unet(latent.to(dtype=unet_dtype),
                                              t,
                                              encoder_hidden_states=uncond_embeddings)["sample"]
            noise_pred = noise_pred.to(dtype=latent.dtype)  # Convert back to fp32
            #
    ...
    def null_optimization(self, inversion_latents):
        ...
        #
        for i in tqdm(range(self.start_step, self.diffusion_steps), disable=True):
    ...
    def attacker(self,
                ...
                for i in range(self.start_step, self.diffusion_steps):
                    ...
                    latent_holder = self.diffusion_step(latent_holder,
                                                        all_uncond_embs[i -
                                                                        self.start_step],
                                                        t, True)
            # Ensure latent dtype matches VAE dtype
            vae_dtype = next(self.diff_model.vae.parameters()).dtype
            fast_render_image = self.diff_model.vae.decode(
                (1 / 0.18215 * latent_holder).to(dtype=vae_dtype))['sample']
        ...
        for _, _ in enumerate(tqdm(range(self.prot_steps), disable=True)):
            controller.loss = 0
            controller.reset()
            ...
            out_image = self.diff_model.vae.decode(
                (1 / 0.18215 * latents).to(dtype=vae_dtype))['sample'][1:]
            #
            ...
            #
            #
            if i == self.diffusion_steps - self.start_step - 1 or i % 10 == 0:
                print(f"Step {i}: Loss={loss.item():.4f}, Adv={adv_loss.item():.4f}")
            #
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def run(self):
        ...
        target_image, _ = get_target_test_images(
            self.target_choice, self.device, self.MTCNN_cropping, self.assets_path)
        with torch.no_grad():
            target_embeddings = self.get_FR_embeddings(target_image)

        for i, (fname, image, path) in enumerate(self.dataloader):
            image_name = fname[0]
            image = image.to(self.device)
            #
            bb_src1 = None
            if self.MTCNN_cropping:
                # Use the provided full path instead of reconstructing it
                path = path[0] 
                # path = self.source_dir + '/' + image_name + '.jpg' # Legacy logic
                img = Image.open(path)
