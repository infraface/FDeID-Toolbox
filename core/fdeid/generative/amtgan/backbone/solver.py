...
# These imports are only needed for training, not inference
# They will be imported lazily in build_model() method
...
class Solver():
    def __init__(self, config, target_image, data_loader=None, inference=None):
        ...
        if inference is not None:
            self.G.load_state_dict(torch.load(inference, map_location=self.device))
        ...
        self.model_zoo_path = config.MODELZOO.PATH
    ...
    def build_model(self):
        ...
        # Import FR models for adversarial training
        # These require the assets directory to be set up (see AMT-GAN README)
        try:
            import sys
            # Add amtgan directory to path for assets import
            amtgan_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            if amtgan_dir not in sys.path:
                sys.path.insert(0, amtgan_dir)
            from assets.models import irse, ir152, facenet
        except ImportError as e:
            raise ImportError(
                "AMT-GAN training requires the 'assets' directory with pretrained FR models. "
                "Please download from: https://drive.google.com/file/d/1Vuek5-YTZlYGoeoqyM5DlvnaXMeii4O8 "
                "and extract to the amtgan directory. Original error: " + str(e)
            )
        ...
        for model in self.train_model_name_list:
            if model == 'ir152':
                ...
                fr_model.load_state_dict(torch.load(os.path.join(self.model_zoo_path, 'ir152.pth'), map_location=self.device))
            ...
            if model == 'irse50':
                ...
                fr_model.load_state_dict(torch.load(os.path.join(self.model_zoo_path, 'irse50.pth'), map_location=self.device))
                fr_model.to(self.device)
                fr_model.eval()
                self.targe_models[model].append(fr_model)
            if model == 'facenet':
                ...
                fr_model.load_state_dict(torch.load(os.path.join(self.model_zoo_path, 'facenet.pth'), map_location=self.device))
                fr_model.to(self.device)
                fr_model.eval()
                self.targe_models[model].append(fr_model)
            if model == 'mobile_face':
                ...
                fr_model.load_state_dict(torch.load(os.path.join(self.model_zoo_path, 'mobile_face.pth'), map_location=self.device))
                fr_model.to(self.device)
                fr_model.eval()
                self.targe_models[model].append(fr_model)
    ...
    def load_checkpoint(self):
        ...
        if os.path.exists(D_A_path):
            self.D_A.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(D_A_path, map_location=self.device).items()})
        ...
        if os.path.exists(D_B_path):
            self.D_B.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(D_B_path, map_location=self.device).items()})
            print('loaded trained discriminator B {}..!'.format(D_B_path))
        H_path = os.path.join(self.checkpoint, 'H.pth')
        if os.path.exists(H_path):
            self.H.load_state_dict(torch.load(H_path, map_location=self.device))
    ...
    def train(self):
        # Lazy import for training-only dependency
        from ..tools import plot as plot_fig
