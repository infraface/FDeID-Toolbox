...
# MeshRenderer requires nvdiffrast (NVIDIA CUDA only)
# We use lazy loading to avoid import errors on AMD GPUs
# HAS_NVDIFFRAST will be set to True only when renderer is successfully imported
HAS_NVDIFFRAST = None  # None = not yet checked, True/False after check
_MeshRenderer = None  # Cached MeshRenderer class


def _get_mesh_renderer():
    """Lazy import of MeshRenderer. Returns (MeshRenderer_class, success_bool)."""
    global HAS_NVDIFFRAST, _MeshRenderer

    if HAS_NVDIFFRAST is not None:
        return _MeshRenderer, HAS_NVDIFFRAST
    ...
    try:
        ...
        # Catch all exceptions: ImportError, RuntimeError (Ninja), OSError, etc.
        _MeshRenderer = None
        HAS_NVDIFFRAST = False
        # Uncomment for debugging:
        # print(f"Note: nvdiffrast/renderer not available: {e}")

    return _MeshRenderer, HAS_NVDIFFRAST

# Lazy import for visualization utilities
_util = None
def _get_util():
    global _util
    if _util is None:
        from .utils_3dmm import util as _util_module
        _util = _util_module
    return _util


class FaceReconModel(BaseModel):
    def __init__(self, opt, use_renderer=None) -> None:
        """
        Initialize FaceReconModel.

        Args:
            opt: Configuration options
            use_renderer: Whether to initialize renderer (requires nvdiffrast).
                         None = auto-detect, True = force enable, False = disable
                         For inference on AMD GPUs, set to False.
        """
        ...
        # Determine if we should use renderer (lazy check)
        MeshRenderer, has_nvdiffrast = _get_mesh_renderer() if use_renderer is not False else (None, False)

        if use_renderer is None:
            use_renderer = has_nvdiffrast
        self._use_renderer = use_renderer and has_nvdiffrast

        if use_renderer and not has_nvdiffrast:
            print("Warning: nvdiffrast not available (requires NVIDIA CUDA). "
                  "Renderer disabled. This is OK for inference - only compute_coeff() is needed.")

        self.parallel_names = self.model_names + (['renderer'] if self._use_renderer else [])
        ...
        # Initialize renderer only if available and requested
        if self._use_renderer and MeshRenderer is not None:
            fov = 2 * np.arctan(opt.center / opt.focal) * 180 / np.pi
            self.renderer = MeshRenderer(
                rasterize_fov=fov, znear=opt.z_near, zfar=opt.z_far,
                rasterize_size=int(2 * opt.center), use_opengl=opt.use_opengl
            )
        else:
            self.renderer = None
    ...
    def to(self, device):
        ...
        if self._use_renderer and self.renderer is not None:
    ...
    def reconstruct_face(self, vertex, feature):
        """Reconstruct face from 3D vertices (requires renderer/nvdiffrast)."""
        if not self._use_renderer or self.renderer is None:
            raise RuntimeError(
                "reconstruct_face() requires nvdiffrast renderer which is not available. "
                "nvdiffrast only works on NVIDIA GPUs with CUDA. "
                "For inference on AMD GPUs, use compute_coeff() instead."
            )
    ...
    def compute_visuals(self, x, pred_face, pred_mask, pred_lm):
        """Compute visualization (requires renderer/nvdiffrast)."""
        if not self._use_renderer:
            raise RuntimeError(
                "compute_visuals() requires nvdiffrast renderer which is not available."
            )
        util = _get_util()
        with torch.no_grad():
