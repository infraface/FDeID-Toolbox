#!/usr/bin/env python3
"""
Generate de-identified face images using naive methods.
...
Usage:
    ...
    python scripts/run_naive_deid.py --dataset lfw --method blur --save_dir /path/to/output
    python scripts/run_naive_deid.py --dataset agedb --method pixelate --block_size 16 --save_dir /path/to/output
    python scripts/run_naive_deid.py --dataset affectnet --split Train --method blur --save_dir /path/to/output
    python scripts/run_naive_deid.py --dataset celebahq --split train --method blur --save_dir /path/to/output

    # Using custom dataset path
    python scripts/run_naive_deid.py --dataset custom --dataset_path /path/to/images --structure flat --method blur --save_dir /path/to/output
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Generate de-identified face images using naive methods',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
  # Using YAML config
  python scripts/run_naive_deid.py --config configs/naive/blur_lfw.yaml
  python scripts/run_naive_deid.py --config configs/naive/blur_lfw.yaml --max_images 100

Supported datasets: {', '.join(supported_datasets)}
Supported structures: {', '.join(supported_structures)}

Examples:
  # LFW dataset with blur
  python scripts/run_naive_deid.py --dataset lfw --method blur --kernel_size 60 --save_dir output/

  # AffectNet Train split
  python scripts/run_naive_deid.py --dataset affectnet --split Train --method blur --save_dir output/

  # CelebA-HQ with pixelate
  python scripts/run_naive_deid.py --dataset celebahq --split train --method pixelate --save_dir output/

  # Custom dataset
  python scripts/run_naive_deid.py --dataset custom --dataset_path /path/to/images --structure flat --method blur --save_dir output/
"""
    )
    ...
    # Method arguments
    parser.add_argument('--method', type=str, default='blur', choices=['blur', 'pixelate', 'mask'],
    ...
    parser.add_argument('--mask_color', type=int, nargs=3, default=[0, 0, 0],
                        help='Mask color RGB (default: 0 0 0)')
...
def main():
    ...
    print("=" * 60)
    print("Face De-identification Pipeline (Naive Methods)")
    print("=" * 60)
